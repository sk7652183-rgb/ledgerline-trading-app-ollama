...
this can be normal if there is no `/` route defined.

It still confirms that the Express server is running.
...
when the browser is running on your local computer and the backend is running on EC2.
...
If available memory is very low and swap usage is high, Ollama can become significantly slower.
...
- Use a smaller model.
- Reduce prompt size.
- Reduce unnecessary chat history.
- Use GPU acceleration for larger models.
...
For production, avoid exposing development servers directly to the internet.
